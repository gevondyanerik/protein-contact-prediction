...
class ESM2ContactAnglePredictorMSA(nn.Module):
    """
    A multitask model based on ESM2 that predicts:
      - A contact map (via a bilinear head and sigmoid)
      - An angle map (via an angle head and tanh scaled by π)
    
    This variant integrates MSA information. When `use_msa` is true, the forward expects a tuple:
         (ref_tokens, msa_tokens)
    The MSA embeddings are fused with the reference embeddings (via concatenation and projection).
    """
    ...
    def forward(self, tokens):
        """
        Args:
            tokens: either a tensor of shape (B, L+2) or a tuple (ref_tokens, msa_tokens)
        Returns:
            Tuple (contact_probs, angle_preds)
        """
        ...
        return contact_probs, angle_preds
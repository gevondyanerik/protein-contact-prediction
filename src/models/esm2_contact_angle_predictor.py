...
class ESM2ContactAnglePredictor(nn.Module):
    """
    A multitask model based on ESM2 that predicts:
      - A contact map (B, L+2, L+2) via a bilinear head and sigmoid.
      - An angle map (B, L+2, L+2) via an angle head and tanh (scaled by π).
    Optionally applies transformer-based fusion using the angle branch.
    """
    def __init__(self, esm_model_name='esm2_t6_8M_UR50D', num_layers_to_freeze=None,
                 feature_fusion=False, fusion_scale=0.5):
        super().__init__()
    ...
    def forward(self, tokens):
        ...
        if self.feature_fusion:
            ...
            if self.transformer_fusion is None or self.transformer_fusion.proj_in.in_features != current_input_dim:
                self.transformer_fusion = TransformerFusion(input_dim=current_input_dim, d_model=64, num_heads=4).to(tokens.device)
